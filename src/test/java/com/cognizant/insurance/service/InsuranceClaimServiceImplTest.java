...
class InsuranceClaimServiceImplTest {
	...
	void testGetAllInsurerDetail() {
		...
		assertEquals("Individual Health Plan",
				insuranceClaimServiceImpl.getAllInsurerDetail().get(0).getInsurerPackageName());
	}
	...
	void testGetInsurerDetail() {
		Optional<InsurerDetail> ins = Optional
				.of(new InsurerDetail(0, "Life Insurance Corporation", "Individual Health Plan", 4000, 10));
		when(insurerDetailRepository.findById(0l)).thenReturn(ins);
		assertEquals("Individual Health Plan",
				insuranceClaimServiceImpl.getInsurerDetail("Individual Health Plan").getInsurerPackageName());
	}
	...
	void testInitiateClaim() {
		...
		treatmentPlanList.add(new TreatmentPlan(1, patientDetail, "Test1", "Package2", "Junior Specialist", 18000,
				"In Progress", "10/03/2021", "10/04/2021"));
